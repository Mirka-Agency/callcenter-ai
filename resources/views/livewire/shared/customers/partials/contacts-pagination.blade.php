@php
    ...
    $pageItems = \App\Support\CompactPaginationWindow::pages(
...
@if ($paginator->hasPages())
    <nav
        ...
        data-tour="customers-contacts-pagination"
    >
        ...
        <span class="relative z-0 hidden overflow-hidden rounded-md border border-zinc-200 dark:border-zinc-700 sm:inline-flex rtl:flex-row-reverse">
            ...
            @foreach ($pageItems as $item)
                @if ($item === '...')
                    <span class="inline-flex items-center border-s border-zinc-200 px-3 py-2 text-sm text-zinc-400 dark:border-zinc-700">...</span>
                @else
                    <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $item }}">
                        @if ($item == $paginator->currentPage())
                            <span aria-current="page" class="inline-flex items-center border-s border-zinc-200 bg-zinc-100 px-3 py-2 text-sm font-medium text-zinc-900 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">{{ $item }}</span>
                        @else
                            <button type="button" wire:click="gotoPage({{ $item }}, '{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" class="inline-flex items-center border-s border-zinc-200 px-3 py-2 text-sm text-zinc-600 transition hover:bg-zinc-50 hover:text-zinc-900 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800" aria-label="{{ __('Go to page :page', ['page' => $item]) }}">{{ $item }}</button>
                        @endif
                    </span>
                @endif
            @endforeach
