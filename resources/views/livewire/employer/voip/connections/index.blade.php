<div class="space-y-8">
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('employer.voip.connections.create') }}" class="saas-btn-primary">اتصال جدید</a>
        </div>
    </div>
    ...
    <div class="grid gap-4">
            ...
            <div class="saas-card space-y-3">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('employer.voip.connections.edit', $connection) }}" class="saas-btn-secondary text-sm">ویرایش</a>
                        <button type="button" wire:click="test({{ $connection->id }})" class="saas-btn-secondary text-sm">تست</button>
                        <button type="button" wire:click="sync({{ $connection->id }})" class="saas-btn-secondary text-sm">همگام‌سازی داخلی‌ها</button>
                        <button type="button" wire:click="regenerateToken({{ $connection->id }})" wire:confirm="{{ __('ui.voip.webhook_regenerate_confirm') }}" class="saas-btn-secondary text-sm">تغییر توکن</button>
        ...
        @empty
            <div class="saas-card text-sm text-zinc-500">هنوز اتصال VoIP ثبت نشده است.</div>
        @endforelse
    </div>
</div>
